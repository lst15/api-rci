...
use crate::domain::models::{ResortPackagesRequest, ResortPackagesResult};
use crate::domain::ports::{RciGateway, SessionRepository};

pub struct GetResortPackagesUseCase {
    session_repository: Arc<dyn SessionRepository>,
    rci_gateway: Arc<dyn RciGateway>,
    retry_policy: RetryPolicy,
}

impl GetResortPackagesUseCase {
    pub fn new(
        session_repository: Arc<dyn SessionRepository>,
        rci_gateway: Arc<dyn RciGateway>,
        retry_policy: RetryPolicy,
    ...
    pub async fn execute(
    ...
    ) -> Result<ResortPackagesResult, AppError> {
        ...
        for attempt in 0..=self.retry_policy.attempts {
            ...
            let response = self.rci_gateway.resort_packages(&session, &request).await?;
            if response.status == 401 || response.status == 403 {
                warn!(
                    username = %session.username,
                    status = response.status,
                    endpoint = "resort_packages",
                    attempt = attempt + 1,
            ...
            let payload = response.payload.ok_or_else(|| {
                AppError::RciUnavailable("missing resort packages payload".into())
            })?;

            return Ok(ResortPackagesResult {
                username: session.username,
